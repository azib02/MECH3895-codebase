...
T_HAND_ON_BOTTLE = np.array(
    [
        [-0.02627674, 0.99923025, -0.02912808, 0.00297862],
        [0.99965076, 0.02618363, -0.00357357, 0.00510153],
        [-0.00280814, -0.02921181, -0.9995693, 0.1212709],
        [0.0, 0.0, 0.0, 1.0],
    ]
)

# Parent = wine_rack_1_main, Child = wine_bottle_1_main
T_BOTTLE_IN_RACK = np.array(
    [
        [0.99752148, -0.06807418, 0.01779902, -0.02092435],
        [0.03358918, 0.68297915, 0.72966516, -0.01381804],
        [-0.06182772, -0.72725881, 0.68357293, 0.23369791],
        [0.0, 0.0, 0.0, 1.0],
    ]
)
...
def run_solver(env, bddl_file=None):
    ...
    print("Phase 2: Placing wine bottle on rack...")

    goal_pose = get_matrix(env, "wine_rack_1_main") @ T_BOTTLE_IN_RACK @ T_HAND_ON_BOTTLE

    move_to_smooth(env, goal_pose, offset=[0, 0, 0.20], steps=100, grip=1.0)
    move_to_smooth(env, goal_pose, offset=[0, -0.10, 0], steps=100, grip=1.0)

    gripper_action(env, cmd=-1.0, steps=40)

    move_to_smooth(env, goal_pose, offset=[0, -0.10, 0.20], steps=100, grip=-1.0)
