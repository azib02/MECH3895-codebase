...
T_HAND_ON_SOUP = np.array(
    [
        [-0.02517364, 0.99962145, 0.01110136, -0.01104641],
...
def run_solver(env, bddl_file=None):
    ...
    move_to_smooth(env, soup_goal, offset=[-0.06, 0, 0.15], steps=100, grip=1.0)
    move_to_smooth(env, soup_goal, offset=[-0.06, 0, 0.02], steps=100, grip=1.0)

    gripper_action(env, cmd=-1.0, steps=40)

    move_to_smooth(env, soup_goal, offset=[-0.06, 0, 0.20], steps=100, grip=-1.0)

    print("Phase 2: Placing cream cheese in basket...")
    ...
    move_to_smooth(env, cheese_goal, offset=[0, 0, 0.10], steps=100, grip=1.0)
    move_to_smooth(env, cheese_goal, offset=[0, 0, 0.04], steps=100, grip=1.0)

    gripper_action(env, cmd=-1.0, steps=40)

    move_to_smooth(env, cheese_goal, offset=[0, 0, 0.20], steps=100, grip=-1.0)
