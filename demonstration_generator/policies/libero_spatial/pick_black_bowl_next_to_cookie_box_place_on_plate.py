...
def nlerp(q1, q2, alpha):
    """Normalised linear interpolation for smooth, shortest-path rotations."""
    if np.dot(q1, q2) < 0:
        q2 = -q2
...
def gripper_action(env, cmd, steps=25):
...
T_BOWL_ON_PLATE = np.array(
    [
        ...
        [-0.02314986, -0.01645033, 0.99959665, 0.04515369],
        [0.0, 0.0, 0.0, 1.0],
    ]
)
...
def run_solver(env, bddl_file=None):
    ...
    bowl_start = get_matrix(env, target_bowl) @ T_HAND_ON_BOWL

    move_to_smooth(env, bowl_start, offset=[0, 0, 0.15], steps=80, grip=-1.0)
    move_to_smooth(env, bowl_start, offset=[0, 0, -0.07], steps=60, grip=-1.0)

    gripper_action(env, cmd=1.0, steps=25)

    move_to_smooth(env, bowl_start, offset=[0, 0, 0.20], steps=60, grip=1.0)

    print("Phase 2: Placing bowl on plate...")

    goal_pose = get_matrix(env, "plate_1_main") @ T_BOWL_ON_PLATE @ T_HAND_ON_BOWL

    move_to_smooth(env, goal_pose, offset=[0, 0, 0.12], steps=80, grip=1.0)
    move_to_smooth(env, goal_pose, offset=[0, 0, 0.01], steps=60, grip=1.0)

    gripper_action(env, cmd=-1.0, steps=25)

    move_to_smooth(env, goal_pose, offset=[0, 0, 0.20], steps=60, grip=-1.0)
