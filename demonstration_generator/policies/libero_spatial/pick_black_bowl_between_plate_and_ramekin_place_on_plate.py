...
def nlerp(q1, q2, alpha):
    """Shortest-path rotation interpolation for fluid twisting motions."""
    if np.dot(q1, q2) < 0:
        q2 = -q2
...
def run_solver(env, bddl_file=None):
    ...
    print(f"Phase 1: Picking up {target_bowl} with twisted grasp...")

    base_grasp_world = get_matrix(env, target_bowl) @ T_HAND_ON_BOWL
    twist = rotation_matrix(-np.pi / 2, [0, 0, 1])
    twisted_grasp_world = base_grasp_world @ twist

    move_to_smooth(env, twisted_grasp_world, offset=[0, 0, 0.15], steps=120)
    move_to_smooth(env, twisted_grasp_world, offset=[0.05, -0.05, -0.02], steps=80)

    gripper_action(env, cmd=1.0, steps=30)

    move_to_smooth(env, twisted_grasp_world, offset=[0, 0, 0.20], steps=80, grip=1.0)

    print("Phase 2: Placing bowl on plate...")

    goal_world = get_matrix(env, "plate_1_main") @ T_BOWL_ON_PLATE @ T_HAND_ON_BOWL

    move_to_smooth(env, goal_world, offset=[0, 0, 0.15], steps=120, grip=1.0)
