...
def nlerp(q1, q2, alpha):
    """Shortest-path rotation interpolation for fluid motion."""
    if np.dot(q1, q2) < 0:
        q2 = -q2
...
def run_solver(env, bddl_file=None):
    ...
    print(f"Phase 1: Picking up {target_bowl} from top drawer...")

    bowl_start_world = get_matrix(env, target_bowl) @ T_HAND_ON_BOWL

    move_to_smooth(env, bowl_start_world, offset=[0, 0, 0.12], steps=100)
    move_to_smooth(env, bowl_start_world, offset=[0.05, 0, -0.13], steps=80)

    gripper_action(env, cmd=1.0, steps=30)

    move_to_smooth(env, bowl_start_world, offset=[0, 0, 0.15], steps=80, grip=1.0)
