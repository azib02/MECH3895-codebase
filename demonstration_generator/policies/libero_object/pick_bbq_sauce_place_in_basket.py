...
# Parent = bbq_sauce_1_main, Child = robot0_right_hand
T_HAND_ON_SAUCE = np.array(
    [
        [0.08092335, -0.99671952, -0.001266, -0.00011819],
        [-0.99662168, -0.08093314, 0.01396626, 0.00149903],
        [-0.01402291, 0.00013153, -0.99990167, 0.12024948],
        [0.0, 0.0, 0.0, 1.0],
    ]
)

# Parent = basket_1_main, Child = bbq_sauce_1_main
T_SAUCE_IN_BASKET = np.array(
    [
        [0.99414865, 0.10529367, -0.02411848, -0.00843131],
        [-0.10546343, 0.99440583, -0.00587449, -0.00513951],
        [0.02336501, 0.00838373, 0.99969185, 0.08571253],
        [0.0, 0.0, 0.0, 1.0],
    ]
)
...
def run_solver(env, bddl_file=None):
    ...
    """
    print("Phase 1: Picking up BBQ sauce...")

    pick_pose = get_matrix(env, "bbq_sauce_1_main") @ T_HAND_ON_SAUCE
    ...
    print("Phase 2: Placing BBQ sauce in basket...")

    goal_pose = get_matrix(env, "basket_1_main") @ T_SAUCE_IN_BASKET @ T_HAND_ON_SAUCE
