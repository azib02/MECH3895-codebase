...
# Parent = milk_1_main, Child = robot0_right_hand
T_HAND_ON_MILK = np.array(
    [
        [-0.0287527, -0.99922508, -0.02687961, -0.02174061],
        [-0.05452703, 0.02841857, -0.9981078, 0.15324799],
        [0.99809823, -0.02723262, -0.05530189, 0.00765211],
        [0.0, 0.0, 0.0, 1.0],
    ]
)

# Parent = basket_1_main, Child = milk_1_main
T_MILK_IN_BASKET = np.array(
    [
        [0.99996363, 0.00334759, -0.00784458, 0.07398039],
...
def run_solver(env, bddl_file=None):
    ...
    """
    print("Phase 1: Picking up milk...")

    milk_start = get_matrix(env, "milk_1_main") @ T_HAND_ON_MILK

    move_to_smooth(env, milk_start, offset=[0, 0, 0.15], steps=100, grip=-1.0)
    move_to_smooth(env, milk_start, offset=[0, 0, 0.00], steps=100, grip=-1.0)

    gripper_action(env, cmd=1.0, steps=40)

    move_to_smooth(env, milk_start, offset=[0, 0, 0.20], steps=100, grip=1.0)

    print("Phase 2: Placing milk in basket...")

    goal_pose = get_matrix(env, "basket_1_main") @ T_MILK_IN_BASKET @ T_HAND_ON_MILK
