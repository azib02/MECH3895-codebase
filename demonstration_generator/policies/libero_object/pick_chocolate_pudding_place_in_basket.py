...
T_HAND_ON_CHOCOLATE_PUDDING = np.array(
    [
        [-0.99961622, 0.02006421, 0.01910082, -0.00450658],
        [0.01959766, 0.99951243, -0.02430703, 0.00187036],
        [-0.01957921, -0.02392337, -0.99952205, 0.0397281],
        [0.0, 0.0, 0.0, 1.0],
    ]
)

# Parent = basket_1_main, Child = chocolate_pudding_1_main
T_CHOCOLATE_PUDDING_IN_BASKET = np.array(
    [
        [7.37464141e-02, -9.95935388e-01, -5.17123636e-02, 0.00075441],
...
def run_solver(env, bddl_file=None):
    ...
    """
    print("Phase 1: Picking up chocolate pudding...")

    pudding_grasp = (
        get_matrix(env, "chocolate_pudding_1_main")
        @ T_HAND_ON_CHOCOLATE_PUDDING
    )

    move_to_smooth(env, pudding_grasp, offset=[0, 0, 0.15], steps=100, grip=-1.0)
    move_to_smooth(env, pudding_grasp, offset=[0, 0, 0.00], steps=100, grip=-1.0)

    gripper_action(env, cmd=1.0, steps=40)

    move_to_smooth(env, pudding_grasp, offset=[0, 0, 0.15], steps=100, grip=1.0)

    print("Phase 2: Placing chocolate pudding in basket...")

    pudding_goal = (
        get_matrix(env, "basket_1_main")
        @ T_CHOCOLATE_PUDDING_IN_BASKET
        @ T_HAND_ON_CHOCOLATE_PUDDING
    )

    move_to_smooth(env, pudding_goal, offset=[0, 0, 0.10], steps=100, grip=1.0)
    move_to_smooth(env, pudding_goal, offset=[0, 0, 0.04], steps=100, grip=1.0)

    gripper_action(env, cmd=-1.0, steps=40)

    move_to_smooth(env, pudding_goal, offset=[0, 0, 0.20], steps=100, grip=-1.0)
