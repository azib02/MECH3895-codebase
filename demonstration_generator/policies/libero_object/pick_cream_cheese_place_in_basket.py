...
T_HAND_ON_CHEESE = np.array(
    [
        ...
        [-0.01957921, -0.02392337, -0.99952205, 0.0697281],
        [0.0, 0.0, 0.0, 1.0],
    ]
)
...
T_CHEESE_IN_BASKET = np.array(
    [
        [7.37464141e-02, -9.95935388e-01, -5.17123636e-02, 0.03775441],
        [9.97276954e-01, 7.36275648e-02, 4.20212650e-03, 0.00079450],
        [-3.77591088e-04, -5.18814402e-02, 9.98653180e-01, 0.14029001],
        [0.0, 0.0, 0.0, 1.0],
    ]
)
...
def run_solver(env, bddl_file=None):
    ...
    cheese_grasp = get_matrix(env, "cream_cheese_1_main") @ T_HAND_ON_CHEESE

    move_to_smooth(env, cheese_grasp, offset=[0, 0, 0.15], steps=100, grip=-1.0)
    move_to_smooth(env, cheese_grasp, offset=[0, 0, 0.00], steps=100, grip=-1.0)

    gripper_action(env, cmd=1.0, steps=40)

    move_to_smooth(env, cheese_grasp, offset=[0, 0, 0.15], steps=100, grip=1.0)

    goal_cheese = get_matrix(env, "basket_1_main") @ T_CHEESE_IN_BASKET @ T_HAND_ON_CHEESE

    move_to_smooth(env, goal_cheese, offset=[0, 0, 0.10], steps=100, grip=1.0)
    move_to_smooth(env, goal_cheese, offset=[0, 0, 0.04], steps=100, grip=1.0)

    gripper_action(env, cmd=-1.0, steps=40)

    move_to_smooth(env, goal_cheese, offset=[0, 0, 0.20], steps=100, grip=-1.0)
